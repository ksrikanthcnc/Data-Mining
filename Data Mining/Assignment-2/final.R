...
library(pROC)
library(neuralnet)
data <- read_csv("Link to R/Assignment-2/finalfact.csv")
...
cat("\nEntropy with 80:20\n")
matr(ent_ratio)
ent_fold <- train(incomeLow~., data = data, method = "rpart",
                  parms = list(split = "information"),
                  trControl = fold,
                  tuneLength = 10)
...
cat("\ngini with 80:20\n")
matr(gini_ratio)
gini_fold <- train(incomeLow~., data = data, method = "rpart",
                   parms = list(split = "gini"),
                   trControl = fold,
                   tuneLength = 10)
...
cat("~~~Naive Bayes~~~~~\n")
nb_data <- data
income = ifelse( data$income == 1, "Low", "High")
#str(data)
...
threshold=0.01)
plot(ann)
predicted_ <- compute(ann, test[,1:22], rep = 1)
