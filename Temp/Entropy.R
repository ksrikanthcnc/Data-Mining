...
pred_test_ent <- predict(tree_ent, test, type = "class")
conf <- table(test$income, pred_test_ent)
conf
acc <- sum(diag(conf))/sum(conf)
acc

tree_gini <- rpart(income~., train, method = "class", minsplit = 2, minbucket = 1,
                   parms = list(split = 'gini'))
rpart.plot(tree_gini)

pred_test_gini <- predict(tree_gini, test, type = "class")
conf <- table(test$income, pred_test_gini)
conf
acc <- sum(diag(conf))/sum(conf)
acc
