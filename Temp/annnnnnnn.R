...
intrain <- createDataPartition(y = d$fnlwgt, p = 0.8, list = FALSE)
train <- d[intrain,]
test <- d[-intrain,]

#View(d)
n <- names(d)
f <- as.formula(paste("Low + High~", paste(sprintf("`%s`", n[!n %in% "Low" & !n %in% "High"]), collapse="+")))
...
table(predicted[,1],test$Low)
conf <- table(predicted[,1],test$Low)
print(conf)
